...
/**
  ...
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.executeActionsEmail(realmName, userId, { verifyEmailAction: true, updateProfileAction: true, updatePasswordAction: true, configureTOTPAction: true })
        .then(() => {
          console.log('success')
      })
    })
...
function executeActionsEmail (c) {
  return function executeActionsEmail (realmName, userId, options) {
    return new Promise((resolve, reject) => {
      ...
      if (options.redirectUri && options.redirectUri !== '') {
        if (options.clientId !== '') {
          qs.redirectUri = options.redirectUri;
          qs.clientId = options.clientId;
        } else {
          return reject(new Error('clientId is not set but redirectUri is. Both MUST go together'));
        }
      }
      if (options.lifespan) {
        qs.lifespan = options.lifespan;
      }
