...
/**
  ...
  @param {string} realmName - The name of the realm(not the realmID) - ex: master,
  @param {object} payload { temporary: boolean , value : string , userId : userId} - The JSON representation of the fields to update for the password - This must include the userId field
  @returns {Promise} A promise that resolves.
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.resetPassword(realmName, { temporary: boolean , value : string , userId : userId})
        .then(() => {
          console.log('success')
      })
    })
