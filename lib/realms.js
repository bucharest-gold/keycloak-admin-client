...
/**
  A function to get all the keys from a realms
  @param {string} [realmName] - The name of the realm(not the realmID) to find.
  @returns {Promise} A promise that will resolve with the an object containing an Array of keys
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.realms.keys(realmName)
        .then((keys) => {
...
function keys (client) {
  return function keys (realmName) {
    return new Promise((resolve, reject) => {
      const req = {
        url: `${client.baseUrl}/admin/realms/${realmName}/keys`,
        auth: { bearer: privates.get(client).accessToken },
        json: true
      };
